...
### Cycle 5 — 2026-06-07 20:00 — GitHub harvest (大井の要望)
- **Plan**: 高star AI/agentリポを収集→解析→自システム強化提案(大井要望)。`github-harvest` workflow化。
- **Do**: gh(認証済ooiyuei)で4リポ収集(SuperAGI 17.5k★/UltraMemory/agent-memory-desktop/Long-Term-Memory-API)。Opus5並列で**実スクリプト(priority_filler/consolidate/embed_wiki/harvest)を読んだ上で**16パターン抽出・feasibility×value採点 → 提案doc `08_openclaw/harvest/2026-06-07-github-harvest-proposals.md`。
- **Check**: 全リポ「アルゴリズムだけ移植・TS/Postgres/Electronはcloneしない」判定。推奨5: A Cost Guard / B Sleep Cycle(episodic→semantic consolidation) / C RRF+MMR re-rank(rank_wiki.ps1) / D telemetry scoreboard / E frontmatter schema。skip: dry-run forgetting(既存)・token budget(8GBにoverkill)等。#1=Cost Guard。
- **Act/次**: #1を即cycle6で実装。大井判断flag: guard上限値 / qwenにentity自動編集を許すか(pattern B) 。

### Cycle 6 — 2026-06-07 19:56 — Cost Guard (harvest #1採用)
- **Plan**: priority_fillerは生成「後」にdedup+consolidationで掃除するが生成「前」の硬いゲートが無い→洪水を構造的に不可能にする日次レート上限を入れる。
- **Do**: `dispatch.ps1`(全producerの唯一のchokepoint)に Cost Guard追加。`.guard_YYYYMMDD.txt` 日次カウンタ。backpressure(在庫150)の直後でcap判定($guardMax=**60/日**・super bypass)、実enqueue成功時のみincrement。ASCII追記(Edit encoding保持確認済)。
- **Check**: priority_filler実行=正常parse・dedup正常(4件skip=increment無し)。新規dispatch=enqueue成功→guard count=1。**実enqueueのみカウントを実証**。backpressure(stock)+dedup(12h)+cost-guard(rate)の三重防御完成。
- **Act/次**: guard稼働。**大井: $guardMax=60/日 は暫定。tune可(dispatch.ps1冒頭)**。次cycle7候補=B Sleep Cycle consolidation(脳のepisodic→semantic昇華・brain-as-human-brain深化) or D telemetry(guard/throughput可視化)。
- **三重防御の整理**: backpressure=「溜まり過ぎ」/ dedup=「同一12h」/ cost-guard=「日次出し過ぎ」。flood(437/晩)は二度と起きない。
